(function ($, undefined) {
    ...
    $(function () {
        ...
        var CLIENT_TYPE = (window.CLIENT_TYPE === 'listener') ? 'venter' : 'listener';
        var OTHER_CLIENT_TYPE = window.CLIENT_TYPE;
...
*/
        ...
        var elizaInitials = ["How do you do.  Please tell me your problem.",
        // additions (not original)
        "Please tell me what's been bothering you.", "Is something troubling you ?"];

        var elizaFinals = ["Goodbye.  It was nice talking to you.",
        // additions (not original)
        ...
        var elizaPres = ["dont", "don't", "cant", "can't", "wont", "won't", "recollect", "remember", "recall", "remember", "dreamt", "dreamed", "dreams", "dream", "maybe", "perhaps", "certainly", "yes", "machine", "computer", "machines", "computer", "computers", "computer", "were", "was", "you're", "you are", "i'm", "i am", "same", "alike", "identical", "alike", "equivalent", "alike"];

        var elizaPosts = ["am", "are", "your", "my", "me", "you", "myself", "yourself", "yourself", "myself", "i", "you", "you", "I", "my", "your", "i'm", "you are"];
...
*/
        ...
        ["xnone", 0, [
            ...
            ["hey", 0, [
                ["*", ["How are you.  Please state your problem.", "Hi.  What seems to be your problem ?"]]
            ]],
            ["computer", 50, [
        ...
        var elizaPostTransforms = [/ old old/g, " old", /\bthey were( not)? me\b/g, "it was$1 me", /\bthey are( not)? me\b/g, "it is$1 me", /Are they( always)? me\b/, "it is$1 me", /\bthat your( own)? (\w+)( now)? \?/, "that you have your$1 $2 ?", /\bI to have (\w+)/, "I have $1", /Earlier you said your( own)? (\w+)( now)?\./, "Earlier you talked about your $2."];

        var ElizaBot = function ElizaBot(noRandomFlag) {
        ...
        comm.handler("join", function (otherUserId, type, geoInfo) {
            addMessage('Eliza', "Got message from " + type, 'yellow-row');
            if (type !== CLIENT_TYPE) {
                addMessage('Eliza', "Joined as " + CLIENT_TYPE, 'yellow-row');
            }
        });

        comm.on('connect', function (first, userId) {
            addMessage('Eliza', first ? 'Connected' : 'Reconnected', 'yellow-row');
            comm.request("join", {
                type: CLIENT_TYPE,
            }, function (result) {
                addMessage('Eliza', 'Got queued', 'yellow-row');
            });
            Chat.create(userId); // creating chat with our userId
        });

        comm.on('disconnect', function () {
            addMessage('Eliza', 'Eliza has been disconnected. Trying to reconnect...', 'yellow-row');
        });
        ...
        comm.handler("msg", function (type, message) {
            if (type != CLIENT_TYPE) {
                comm.request("msg", eliza.transform(message), function (data) {
                    if (data !== true) {
        ...
        comm.handler("partRequest", function (type) {
            addMessage( 'Eliza', 'Chat partner disconnected, please wait while we find you a new ' + OTHER_CLIENT_TYPE + '.', 'yellow-row');
        });

        comm.handler("partDisconnect", function (type) {
            addMessage('Eliza', 'Partner has left the chat.', 'yellow-row');
        });
